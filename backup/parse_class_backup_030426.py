...
class ConstructionBillOCR:
    OCR_PROMPT = """
    Ты OCR. Перепиши ТАБЛИЦЫ с изображения в markdown.
    ...
    Всегда начинай вывод с заголовка ОСНОВНОЙ таблицы:
    | № п/п | Наименование | Ед. изм. | Кол-во |
    |-------|--------------|----------|--------|
    Только после этого — строки основной таблицы.
    ...
    OCR_PROMPT_STAMP = """
    Ты OCR. Перепиши ТАБЛИЦУ ИЗ НИЖНЕЙ ЧАСТИ ИЗОБРАЖЕНИЯ в markdown.

    КРИТИЧЕСКИ ВАЖНО:
    1. НЕ объединяй строки. НЕ изменяй текст. НЕ пропускай строки. НЕ добавляй от себя.
    2. Обрабатывай ТОЛЬКО НИЖНЮЮ ТАБЛИЦУ (штамп, реквизиты).
    3. Ищи таблицу в левом/правом нижнем углу изображения.
    4. Выведи ВСЕ строки таблицы, какие видишь, без фиксированного заголовка.

    Формат вывода:
    Используй markdown-таблицу. Выведи все ячейки, какие есть.
    Примеры возможных ячеек: "Изм.", "Кол. уч.", "Лист", "№ док", "Подп.", "Дата", "Стадия", "Листов".
    Пример вывода:
    | Изм. | Кол. уч. | Лист | № док | Подп. | Дата | Стадия | Листов |
    |------|----------|------|-------|-------|------|--------|--------|
    | 3 | 1 | 1 | 0.1264-КМ1.ВР | И.И.Петров | 01.01.2024 | РД | 12 |
    | | | | | | | | |
    | Шифр | | | | | | | |
    | 163.083.Р.0/0.1264-КМ1.ВР | | | | | | | |
    """.strip()
    ...
    def _fill_file_type(self, df: pd.DataFrame, file_type: str) -> pd.DataFrame:
        """
        Заполняет столбец _file_type для строк, где есть _header или _subheader.
        """
    ...
    def _save_to_excel_header(self, df: pd.DataFrame, path: str):
        """
        Сохраняет Excel с заголовками разделов, подразделов и шифром.
        """
        from openpyxl.styles import Font

        if df.empty:
            df = pd.DataFrame([{"Раздел": "НЕТ РАЗДЕЛОВ", "Подраздел": "", "Шифр": ""}])
        ...
        if header_rows.empty:
            header_rows = pd.DataFrame([{"Раздел": "НЕТ РАЗДЕЛОВ", "Подраздел": "", "Шифр": ""}])
        else:
            ...
            header_rows['Шифр'] = header_rows.get('_file_type', '')

        # Создаём итоговый DataFrame
        final_df = header_rows[['Раздел', 'Подраздел', 'Шифр']].copy()
        ...
        with pd.ExcelWriter(path, engine="openpyxl") as writer:
            ...
            ws.column_dimensions['C'].width = 30  # Шифр
            ...
            for row_idx in range(2, len(final_df) + 2):  # Пропускаем заголовок (строка 1)
                ...
                cell_c = ws.cell(row=row_idx, column=3)  # Шифр
