...
class ConstructionBillOCR:
        ...
        with self._log_lock:
            ...
            if "message" in data:
                ...
                if isinstance(msg, dict):
                    ...
                    self.logger(f"  🔧 content length: {len(content)}, first 200: {repr(content[:200])}")
    ...
    def _detect_markdown_sections(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Находит разделы по **жирному** тексту.
        Только помечает строки-разделы, без пропагации.
        """
        if df.empty:
            return df
        ...
        for idx in df.index:
            ...
            if re.match(r'^\*\*.+\*\*$', name):
                ...
                # Классификация: цифра = заголовок, иначе = подзаголовок
    ...
    def _extract_file_type(self, raw_text: str) -> str:
        """
        Извлекает тип файла (шифр) из сырого текста (например, 0092.049.Р.13/1.0004.УКПГ.045.3856.1226-КМ.ВР(1611)).
        Паттерн: 4 цифры.3 цифры. (далее любые символы).
        """
        lines = raw_text.strip().splitlines()

        # 🔥 Ищем по всем строкам (не только последние)
        for line in lines:
            line = line.strip()

            # 🔍 Паттерн: 4 цифры.3 цифры. (далее любые символы)
            # Пример: 0092.049.Р.13/1.0004.УКПГ.045.3856.1226-КМ.ВР(1611)
    ...
    def _extract_subobject_from_raw(self, raw_text: str) -> str:
        """
        Извлекает подобъект из сырого текста первой страницы.
        Подобъект: строка с **жирным** текстом между шифром и 'Примечание:'.
        """
        lines = raw_text.strip().splitlines()
        subobject = ""
        ...
        start_idx = -1
        for i, line in enumerate(lines):
            if self.document_type and self.document_type in line:
                start_idx = i
                break
        ...
        # 🔍 Ищем **...** после шифра, но до 'Примечание:'
        for j in range(start_idx + 1, len(lines)):
            ...
            # Ищем **жирный** текст в строке
            match = re.search(r'\*\*(.*?)\*\*', line)
            if match:
                subobject = match.group(1).strip()
                break  # Берём первый найденный **...** после шифра

        return subobject

    def _ocr_page(self, page: Image.Image, page_number: int = 1) -> pd.DataFrame:  # Добавим page_number
        raw = self._ocr_from_image(page)

        if not raw:
            raise ValueError("Empty response from Ollama")

        # 🔥 Извлекаем информацию ТОЛЬКО с первой страницы
        if page_number == 1:
            # Извлекаем шифр
            file_type = self._extract_file_type(raw)
            if file_type:
                self.document_type = file_type
                self.logger(f"  🏷️ Найден шифр: {file_type}")

            # Извлекаем подобъект
            subobject = self._extract_subobject_from_raw(raw)
            if subobject:
                self.subobject = subobject
                self.logger(f"  🏷️ Найден подобъект: {subobject}")

        df = self._parse_ocr_output(raw)

        # 🔥 Добавляем тип файла как атрибут страницы (для совместимости)
        df['_page_file_type'] = self.document_type if page_number == 1 else ""
    ...
    def _merge_multiline_subheaders(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        1. Текст начинается с маленькой буквы
        2. Пустые колонки "Ед. изм." и "Кол-во"
        3. Не является разделом (_is_section=False)
        """
        if df.empty:
            return df
        ...
        while i < len(rows):
            ...
            # 🔥 Если это подзаголовок (раздел без цифры)
            if cur_is_section and cur_subheader and not re.match(r'^\d', cur_name):
                combined_subheader = cur_name
                j = i + 1

                # Ищем строки-продолжения
                while j < len(rows):
                    nxt = rows[j]
                    nxt_name = str(nxt.get("Наименование", "")).strip()
                    ...
                    # 🔥 Критерии продолжения подзаголовка:
                    # 1. Не раздел
                    # 2. Нет данных (пустые Ед. изм. и Кол-во)
                    # 3. Начинается с маленькой буквы ИЛИ с пробела/тире
                ...
                merged.append(new_row)
                i = j  # Перескакиваем через склеенные строки
    ...
    def _propagate_headers_across_pages(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Вызывается ПОСЛЕ объединения всех страниц.
        """
        if df.empty:
            return df
        ...
        for idx in df.index:
            ...
            if is_section:
                ...
                if header_val:  # Заголовок с цифрой
                    current_header = header_val
                    current_subheader = ''  # Сброс при новом заголовке
                elif subheader_val:  # Подзаголовок без цифры
                    current_subheader = subheader_val
                    # Заголовок НЕ сбрасываем — подзаголовок внутри заголовка

                # Для строк-разделов тоже заполняем поля
                df.at[idx, '_header'] = current_header if not header_val else header_val
                df.at[idx, '_subheader'] = current_subheader if not subheader_val else subheader_val
            else:
                # Обычная строка — пропагируем текущие значения
    ...
    def _remove_before_first_header(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Удаляет все строки до первого ЗАГОЛОВКА (с цифрой) во всём DataFrame.
        Подзаголовки (без цифры) не считаются точкой отсечения.
        """
        if df.empty:
            return df
        ...
        # Ищем первую строку, которая является заголовком (с цифрой)
        first_header_idx = None
        for idx in df.index:
            if df.at[idx, '_is_section']:
                name = str(df.at[idx, 'Наименование']).strip()
                # Проверяем, что это заголовок (начинается с цифры)
    ...
    def _save_to_excel(self, df: pd.DataFrame, path: str):
        ...
        with pd.ExcelWriter(path, engine="openpyxl") as writer:
            ...
            for row_idx, df_idx in enumerate(df.index, start=2):
                if df.at[df_idx, '_is_section'] and re.match(r'^\d', str(df.at[df_idx, 'Наименование'])):
    ...
    def _save_to_excel_header(self, df: pd.DataFrame, path: str):
        """
        Сохраняет Excel с заголовками разделов, подразделов, шифром и подобъектом.
        """
        from openpyxl.styles import Font

        if df.empty:
            df = pd.DataFrame([{"Раздел": "НЕТ РАЗДЕЛОВ", "Подраздел": "", "Тип документа": "", "Подобъект": ""}])
        ...
        if header_rows.empty:
            header_rows = pd.DataFrame(
                [{"Раздел": "НЕТ РАЗДЕЛОВ", "Подраздел": "", "Тип документа": "", "Подобъект": ""}])
        else:
            ...
            # 🔥 Заполняем Тип документа и Подобъект из атрибутов класса
            header_rows['Тип документа'] = self.document_type
            header_rows['Подобъект'] = self.subobject

        # Создаём итоговый DataFrame
        final_df = header_rows[['Раздел', 'Подраздел', 'Тип документа', 'Подобъект']].copy()
        ...
        with pd.ExcelWriter(path, engine="openpyxl") as writer:
            ...
            # Настройка ширины колонок
            ws.column_dimensions['A'].width = 50  # Раздел
            ws.column_dimensions['B'].width = 50  # Подраздел
            ws.column_dimensions['C'].width = 25  # Тип документа
            ws.column_dimensions['D'].width = 30  # Подобъект

            # Выделение заголовков жирным (все строки — заголовки/подразделы)
            bold_font = Font(bold=True)

            for row_idx in range(2, len(final_df) + 2):  # Пропускаем заголовок (строка 1)
                cell_a = ws.cell(row=row_idx, column=1)  # Раздел
                cell_b = ws.cell(row=row_idx, column=2)  # Подраздел
                cell_c = ws.cell(row=row_idx, column=3)  # Тип документа
                cell_d = ws.cell(row=row_idx, column=4)  # Подобъект
                cell_a.font = bold_font
                cell_b.font = bold_font
                # cell_c.font = bold_font # Тип документа не выделяем жирным, только значения
                # cell_d.font = bold_font # Подобъект не выделяем жирным, только значения

        self.logger(f"✅ Сохранено заголовков: {len(final_df)} строк в {path}")
    ...
    def _process_single_page(self, page_idx: int, page: Image.Image, total_pages: int) -> Optional[pd.DataFrame]:
        try:
            ...
            # 🔥 Передаём номер страницы в _ocr_page
    ...
    def save_to_excel(self, output_path: Optional[str] = None):
        ...
        self._save_to_excel_header(self._df, path)  # <- только заголовки, подразделы, шифр и подобъект