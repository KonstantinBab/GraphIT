...
class ConstructionBillOCR:
    ...
    def _ocr_page(self, page: Image.Image, page_number: int = 1) -> pd.DataFrame:  # Добавили page_number
        ...
        if page_number == 1:  # 🔥 Условие: только первая страница
            ...
            file_type = self._extract_file_type_from_raw_stamp(raw_stamp)  # Нужно будет создать этот метод
            if file_type:
                self.logger(f"  🏷️ Найден шифр из штампа: {file_type}")
    ...
    def _build_summary_row(self, df: pd.DataFrame) -> list:
        """
        Строит одну строку-сводку в виде списка кортежей:
        [
            ('<номер проекта>', '<наименование сооружения>', 1, 1),
            ('Марка РД', '<шифр>', 2, 1),
            ('Раздел ВР1', '<первый заголовок раздела>', 3, -1)
        ]
        """
        summary = []

        # 1. Номер проекта и Наименование сооружения
        # Берём первое непустое значение из всего df (они одинаковы)
        proj_num = df['_project_number'].dropna().iloc[0] if not df['_project_number'].dropna().empty else ""
        proj_name = df['Наименование зданий, сооружений, систем и установок'].dropna().iloc[0] if not df['Наименование зданий, сооружений, систем и установок'].dropna().empty else ""
        if proj_num and proj_name:
            summary.append((proj_num, proj_name, 1, 1))

        # 2. Марка РД (шифр)
        file_type = df['_file_type'].dropna().iloc[0] if not df['_file_type'].dropna().empty else ""
        if file_type:
            summary.append(("Марка РД", file_type, 2, 1))

        # 3. Первый заголовок раздела (первый _header, где он не пустой)
        first_header = None
        for _, row in df.iterrows():
            if row.get('_header') and isinstance(row['_header'], str) and row['_header'].strip():
                first_header = row['_header'].strip()
                break
        if first_header:
            summary.append(("Раздел ВР1", first_header, 3, -1))
    ...
    def _save_to_excel_header(self, df: pd.DataFrame, path: str):
        ...
        # Фильтруем строки с заголовками
        header_rows = df[
        ...
        if header_rows.empty:
            ...
            header_rows['Тип файла'] = header_rows.get('_file_type', '')
            header_rows['Номер проекта'] = header_rows.get('_project_number', '')
            header_rows['Наименование зданий...'] = header_rows.get(
                'Наименование зданий, сооружений, систем и установок', '')

            # 🔥 Генерируем сводку ОДИН раз
            summary_list = self._build_summary_row(df)
            # Преобразуем в строку (чтобы Excel мог сохранить)
            summary_str = str(summary_list)  # Например: "[('3856', 'Площадка...', 1, 1), ...]"
            header_rows['Сводка'] = summary_str

        # Создаём final_df
        cols_order = [
            'Раздел', 'Подраздел', 'Тип файла', 'Номер проекта',
            'Наименование зданий...', 'Сводка'
        ]
        ...
        with pd.ExcelWriter(path, engine="openpyxl") as writer:
            ...
            ws.column_dimensions['F'].width = 80  # Сводка — широкая

            # Жирный шрифт для строк данных
            bold_font = Font(bold=True)
            for row_idx, _ in enumerate(final_df.index, start=2):
                for col_idx in range(1, len(cols_order) + 1):
                    cell = ws.cell(row=row_idx, column=col_idx)
                    cell.font = bold_font
    ...
    def _process_single_page(self, page_idx: int, page: Image.Image, total_pages: int) -> Optional[pd.DataFrame]:
        try:
            ...
            # ПАРСИМ ТОЛЬКО OCR ВЫВОД
            df_page = self._parse_ocr_output(raw)  # <-- df_page: только ["№ п/п", "Наименование", "Ед. изм.", "Кол-во"]
    ...
    def _load_lookup_table(self) -> Optional[pd.DataFrame]:
        ...
        # 🔥 Опционально: преобразуем 'Диапазон кодов, код' в числовой тип, если он строковый
        # Это может потребоваться, если диапазон представлен как '1234' или '1234-5678'
        # Пока оставим как есть, будем парсить при поиске.
