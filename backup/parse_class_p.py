...
class ConstructionBillOCR:
    ...
    def process(self) -> pd.DataFrame:
        ...
        if not all_dfs:
            ...
            # 2. Слияние многострочных наименований — ОТКЛЮЧЕНО (сохраняем как есть)
            # self._df = self._merge_multiline_names(self._df)
            # self.logger(f"   🔗 Слияние работ завершено: {len(self._df)} строк")

            # 3. Слияние многострочных подзаголовков — ОТКЛЮЧЕНО
            ...
            # 6. Удаление строк до первого заголовка — ОТКЛЮЧЕНО
