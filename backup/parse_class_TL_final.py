...
class ConstructionBillOCR:
    ...
    ═══════════════════════════════════════════════════════════════
    ВИЗУАЛЬНЫЕ ПРИЗНАКИ РАЗМЕТКИ
    ═══════════════════════════════════════════════════════════════

    ОКС (объект капитального строительства):
    - Объединённые ячейки в самом верху таблицы, занимают всю ширину
    - Оберни в ***тройные звёздочки***
    - Пример: | | ***Жилой дом №5*** | | |

    Раздел — текст с ПОДЧЁРКИВАНИЕМ (линия под текстом):
    - Главный признак: текст ПОДЧЁРКНУТ
    - Подчёркнутый текст может быть также жирным — ключевой признак именно ПОДЧЁРКИВАНИЕ
    - Обычный жирный текст БЕЗ подчёркивания — НЕ раздел
    - Оберни в **двойные звёздочки**
    - Пример: | | **Каркас здания** | | |
    - Пример: | 14 | **Ведомость объемов специальных работ** | | |

    Подраздел — строка с СЕРЫМ ФОНОМ (заливкой):
    - Строка визуально выделена серым/светлым фоном
    - Подраздел, не может быть ПОДЧЁРКНУТ, если он подчёркнут это РАЗДЕЛ
    - НО: если текст пустой, или начинается с "-" или "—" — это НЕ подраздел, пиши как обычную строку
    - Оберни в *одинарные звёздочки*
    - Пример: | | *Монтаж металлоконструкций* | | |
    - НЕ подраздел: | | - крепёжные элементы | | |

    ═══════════════════════════════════════════════════════════════
    ЧТО НЕ ВЫДЕЛЯТЬ (обычные позиции)
    ═══════════════════════════════════════════════════════════════

    - Строки с заполненными Ед. изм. или Кол-во — всегда обычные
    - Строки без подчёркивания и без серого фона — обычные
    - Строки с серым фоном но начинающиеся с "-" или "—" — обычные
    ...
    | 1 | Монтаж колонн одноветвевых | шт | 10 |
    ...
    def _image_to_base64(self, img: Image.Image, max_long_side: int = 2048) -> str:
        ...
        buf = BytesIO()
        # JPEG значительно легче PNG → быстрее передаётся и обрабатывается
    ...
    def _detect_markdown_sections(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        ***Текст*** = ОКС (объект капитального строительства)
        **Текст**   = Раздел (подчёркнутый текст в оригинале)
        *Текст*     = Подраздел (серый фон в оригинале)
        ...
        for idx in df.index:
            ...
            # ═══════════════════════════════════════════════════════════════
            # ОКС: ***Текст*** (тройные звёздочки)
            # ═══════════════════════════════════════════════════════════════

            if re.match(r'^\*\*\*.+\*\*\*$', name):
                df.at[idx, '_is_section'] = True
                clean_name = re.sub(r'^\*{3}(.+?)\*{3}$', r'\1', name).strip()
                df.at[idx, 'Наименование'] = clean_name
                df.at[idx, '_oks'] = clean_name
                oks_count += 1
                continue

            # ═══════════════════════════════════════════════════════════════
            # РАЗДЕЛ: **Текст** (двойные звёздочки = подчёркнутый текст)
            # ═══════════════════════════════════════════════════════════════

            if re.match(r'^\*\*[^*].+[^*]\*\*$', name):
                df.at[idx, '_is_section'] = True
                clean_name = re.sub(r'^\*{2}(.+?)\*{2}$', r'\1', name).strip()
                df.at[idx, 'Наименование'] = clean_name
            ...
            # ═══════════════════════════════════════════════════════════════
            # ПОДРАЗДЕЛ: *Текст* (одинарные звёздочки = серый фон)
            # Пропускаем если текст начинается с "-" или "—"
            # ═══════════════════════════════════════════════════════════════

            if re.match(r'^\*[^*].+[^*]\*$', name):
                clean_name = re.sub(r'^\*(.+?)\*$', r'\1', name).strip()
                # Строки с "-" в начале — не подразделы
        ...
        self.logger(f"🔖 Найдено: ОКС={oks_count}, разделов={header_count}, подразделов={subheader_count}")
        return df
        ...
        while i < len(rows):
            ...
            if cur_num:
                ...
                while j < len(rows):
                    ...
                    # 🔥 Не склеиваем разделы/подразделы с предыдущей строкой
                    if nxt.get("_is_section", False):
                        break

                    if not nxt_num and nxt_name:
                        if re.match(r'^\d+\s+[А-ЯЁA-Z]', nxt_name):
                            break
                        # Не склеиваем если текст в markdown-разметке (* или **)
                        if re.match(r'^\*', nxt_name):
                            break
    ...
    def _merge_split_sections(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Склеивает разделы/подразделы, разбитые между страницами.
        Если строка-раздел без № п/п следует за другой строкой-разделом того же уровня — объединяем текст.
        То же для ОКС и подразделов.
        """
        ...
        for i in range(1, len(rows)):
            ...
            cur_is_section = cur.get("_is_section", False)
            prev_is_section = prev.get("_is_section", False)

            if not cur_is_section or not prev_is_section:
                merged.append(cur)
                continue

            # Определяем уровень: oks > header > subheader
            cur_level = "oks" if cur.get("_oks") else ("header" if cur.get("_header") else ("subheader" if cur.get("_subheader") else ""))
            prev_level = "oks" if prev.get("_oks") else ("header" if prev.get("_header") else ("subheader" if prev.get("_subheader") else ""))

            # Одинаковый уровень — склеиваем
            if cur_level and cur_level == prev_level:
                cur_name = str(cur.get("Наименование", "")).strip()
                prev_name = str(prev.get("Наименование", "")).strip()

                # Склеиваем только если текущий начинается с маленькой буквы или спецсимвола
                if cur_name and (cur_name[0].islower() or cur_name.startswith(('(', '-', '–', '—'))):
                    combined = prev_name + " " + cur_name
                    prev["Наименование"] = combined
                    # Обновляем соответствующее поле
                    if cur_level == "oks":
                        prev["_oks"] = combined
                    elif cur_level == "header":
                        prev["_header"] = combined
                    elif cur_level == "subheader":
                        prev["_subheader"] = combined
                    self.logger(f"   🔗 Склеен {cur_level}: «{combined}»")
                    continue

            merged.append(cur)

        result_df = pd.DataFrame(merged).reset_index(drop=True)
        diff = len(rows) - len(result_df)
        if diff > 0:
            self.logger(f"🔗 Склеено разделов между страницами: {diff}")
        return result_df
    ...
    def _propagate_headers_across_pages(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Пропагирует ОКС, заголовки и подзаголовки по всему DataFrame.
        """
        if df.empty:
            return df
        ...
        for idx in df.index:
            ...
            if is_section:
                ...
                df.at[idx, '_oks'] = current_oks
                df.at[idx, '_header'] = current_header
                df.at[idx, '_subheader'] = current_subheader

        self.logger(f"🔗 Пропагация завершена (ОКС → Раздел → Подраздел)")
        return df

    def _remove_before_first_section(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Удаляет строки до первого ОКС или раздела.
        Если есть ОКС — берём от него. Если нет — от первого раздела.
        """
        if df.empty:
            return df
        ...
        # Ищем первый ОКС или раздел
        first_idx = None
        for idx in df.index:
            if df.at[idx, '_oks'] or df.at[idx, '_header']:
                first_idx = idx
                break

        if first_idx is not None:
            rows_before = df.index.get_loc(first_idx)
            if rows_before > 0:
                df = df.iloc[first_idx:].reset_index(drop=True)
                self.logger(f"🗑️ Удалено {rows_before} строк до первого ОКС/раздела")
    ...
    def _save_to_excel(self, df: pd.DataFrame, path: str):
        ...
        with pd.ExcelWriter(path, engine="openpyxl") as writer:
            cols_order = ["№ п/п", "ОКС", "Раздел", "Подраздел", "Наименование", "Ед. изм.", "Кол-во"]

            final_df = pd.DataFrame()
            for col in cols_order:
                if col == 'ОКС':
                    final_df[col] = df.get('_oks', '')
                elif col == 'Раздел':
            ...
            ws.column_dimensions['B'].width = 30   # ОКС
            ws.column_dimensions['C'].width = 30   # Раздел
            ws.column_dimensions['D'].width = 40   # Подраздел
            ws.column_dimensions['E'].width = 60   # Наименование
            ws.column_dimensions['F'].width = 10   # Ед. изм.
            ws.column_dimensions['G'].width = 10   # Кол-во

            # Стили для разных уровней
            oks_font = Font(bold=True, size=12)
            oks_fill = PatternFill(start_color="B8CCE4", end_color="B8CCE4", fill_type="solid")  # голубой
            ...
            sub_fill = PatternFill(start_color="E8E8E8", end_color="E8E8E8", fill_type="solid")  # серый

            for row_idx, df_idx in enumerate(df.index, start=2):
                is_section = df.at[df_idx, '_is_section']
                oks_val = df.at[df_idx, '_oks']
                header_val = df.at[df_idx, '_header']
                subheader_val = df.at[df_idx, '_subheader']

                if is_section and oks_val:
                    # ОКС — голубой фон, жирный крупный
                    for col_idx in range(1, len(cols_order) + 1):
                        cell = ws.cell(row=row_idx, column=col_idx)
                        cell.font = oks_font
                        cell.fill = oks_fill
                elif is_section and header_val:
                    # Раздел — подчёркнутый жирный, светло-синий фон
                    for col_idx in range(1, len(cols_order) + 1):
                        cell = ws.cell(row=row_idx, column=col_idx)
                        cell.font = header_font
                        cell.fill = header_fill
                elif is_section and subheader_val:
                    # Подраздел — серый фон
                    for col_idx in range(1, len(cols_order) + 1):
                        cell = ws.cell(row=row_idx, column=col_idx)
                        cell.fill = sub_fill
    ...
    def process(self) -> pd.DataFrame:
        ...
        if not all_dfs:
            ...
            # 2. Склейка разделов, разбитых между страницами
            self._df = self._merge_split_sections(self._df)

            # 3. Слияние многострочных наименований (работы/материалы)
            self._df = self._merge_multiline_names(self._df)
            self.logger(f"   🔗 Слияние работ завершено: {len(self._df)} строк")

            # 4. Слияние многострочных подзаголовков
            self._df = self._merge_multiline_subheaders(self._df)

            # 5. Пропагация заголовков по всему документу
            self._df = self._propagate_headers_across_pages(self._df)

            # 6. Построение иерархии (level, parent_id, path)
            self._df = self._build_hierarchy(self._df)

            # 7. Удаление строк до первого ОКС/раздела
            self._df = self._remove_before_first_section(self._df)
